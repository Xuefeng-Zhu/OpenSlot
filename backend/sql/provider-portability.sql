...
-- `refresh-provider-token` must compare provider_connections.updated_at and
-- update the encrypted access/refresh credentials in one conditional database
-- statement. A REST read followed by a primary-key PATCH is not an atomic
-- compare-and-swap and can overwrite a concurrently rotated refresh token.

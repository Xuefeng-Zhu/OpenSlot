...
| `SLOT_HOLD_TOKEN_SECRET` | Optional | Dedicated HMAC secret for short-lived public slot hold tokens. Omit it to fall back to the legacy `BUTTERBASE_FUNCTION_SECRET`, then a domain-separated key derived from `BUTTERBASE_API_KEY`. |
